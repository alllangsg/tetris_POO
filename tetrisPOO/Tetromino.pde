...
class Tetromino implements Cloneable {
 ...
 //Mueve el Tetromino una columna a la derecha
  ...
  // Rotacion 180° en sentido horario.
...
boolean checkLeft() {
  ...
  for (int j = 0; j < 4; j++)
  {
    for (int i = j; i < 16; i += 4) {
      if ((rotaciones[rotacion] & (1 << 15 - i)) != 0) {
        posx = (i%4) + column;
        posy = ((i/4)|0) + row;
        if (table.Memoria.get(posy)[posx-1] != 0)
          print("LEFT");
          return true;
      }
    }
  ...
  boolean checkTranslate (int caso) { //caso =1, es para colision IZQ
    for (int j = 0; j < 4; j++){
    
      for (int i = j; i < 16; i += 4) {
        if (((rotaciones[rotacion] & (1 << 15-i)) != 0) && caso == 1) { //IZQ.
          posx = (i%4) + column;  
          posy = ((i/4)|0) + row;
          if (posy>=0 && (table.Memoria.get(posy)[posx-1] != 0))
            print("good Izq.");
            return true;
        }else if (((rotaciones[rotacion] & (1 << i)) != 0) && caso == 0) { //DER.
          posx = ((15-i)%4) + column;
          posy = (((15-i)/4)|0) + row;
          if (posy>=0 && (table.Memoria.get(posy)[posx+1] != 0))
            print("good der");
